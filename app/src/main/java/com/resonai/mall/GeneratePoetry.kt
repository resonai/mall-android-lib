...
internal object GeneratePoetry {
        ...
        fnBuilderPair.first
            ...
            .addStatement(
                String.format(
                    "return %s(%s) { %s -> %s(%s) }",
                    "combine",
                    (1..n).joinToString { "flow$it" },
                    (1..n).joinToString { "t$it" },
    ...
    fun generateCombinerTupleNFn(n: Int, combiner: String): FunSpec {
        assert(n > 1)
        ...
        return fnBuilderPair.first
            .addKdoc(
                "AUTOGEN(KotlinPoet) generateCombinerTupleNFn(%L, %L)", n, combiner
            )
            .build()
    }
 ...
 */
    ...
    fun generateInputsBuilderNClass(n: Int, maxGenerated: Int = 8): TypeSpec {
        ...
        val insBuilder = TypeSpec.classBuilder("InputsBuilder$n")
            ...
            .addTypeVariables(types)
            .addSuperinterface(InputAddable::class.asClassName().parameterizedBy(tupleType))
    ...
    fun generateInputsNClass(n: Int): TypeSpec {
        ...
        for (possibles in listOf(false, true)) {
            ...
            val transformFn = FunSpec.builder(funName)
                ...
                .addParameter(
                    ParameterSpec.builder(
                        "to", FlowCollector::class.asClassName().parameterizedBy(rType)
                    ).build()
                )
            ...
            builder.addFunction(transformFn)
        }
